...
def train_and_predict_old(xTrain,yTrain,xVal,yVal,modelname,event,modelParams, predictOnData = None):

    if modelname == 'dnn':
        model = DNN_old(modelParams,modelname,event)
    elif modelname == 'cnn':
...
for model in models:
    ...
    for event in events:
        ...
        directory = 'dnn_05_06_PM_July_27_2017'  
        ...
        predictions,history = train_and_predict_old(xTrain,yTrain,xVal,yVal,model,event,modelParams,
                                        predictOnData = xVal)
