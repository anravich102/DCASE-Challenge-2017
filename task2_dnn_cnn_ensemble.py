...
dnn_weight = modelParams['ensemble_dnn_weight']
...
for model in models:
    ...
    if model == 'dnn':
        directory = 'dnn_04_49_PM_July_17_2017'                   # CHANGE THIS!!  
    elif model == 'cnn':
        directory = 'dnn_04_49_PM_July_17_2017'
    ...
    directory = do_feature_extraction( model, event, source_data = False, mode = 'devtest', directory = directory)
    print("Done with  Feature Extraction !")

    # print("Starting Feature Normalization for validation data....")
    # ignore = do_feature_normalization(model, event, directory, source_data = False, mode = 'devtest')
    # print("Done with  Feature Normalization !")

    print("Starting Label Extraction for validation data....")
    ignore = do_label_extraction(directory, model,event, source_data = False, mode = 'devtest',)
    print("Done With Label Extraction!")
...
predictions = do_ensembling(predList, dnn_weight)
...
def do_ensembling(predList, dnn_weight, type = 'average_before'):

   # predList = ['cnn_predictions', 'dnn_predictions']

    a = predictList[0]
    b = predList[1]

    if(a.size > b.size):
        b = np.concatenate((b, np.zeros(a.size-b.size)))

    if(a.size < b.size):
        a = np.concatenate((a, np.zeros(b.size-a.size)))

    return ( (1-dnn_weight)*a + ddnn_weight * b) /2
